...
cv::Point2d computeIntersect(cv::Vec4i a, cv::Vec4i b) {
...
bool isLine(cv::Mat& mat, double& linkScore, double& linkSpace, cv::Point pt1,
		cv::Point pt2, int threshhold, int size, int mode, int procMode,
		bool debug) {
	if (mat.channels() != 1) {
...
bool notRectLineLv2(float angle, float rho) {
	if (rho < 0)
		angle += CV_PI;
	if ((angle >= CV_PI / 3 && angle <= 2 * CV_PI / 3)
			|| (angle >= CV_PI * 4 / 3 && angle <= 5 * CV_PI / 3))
		return false;
	if ((angle >= 0 && angle <= CV_PI / 6)
			|| (angle >= CV_PI * 11 / 6 && angle <= 2 * CV_PI))
		return false;
	return true;
}
struct quadrNode {
...
bool isRealQuadr(cv::Mat pic, cv::Vec4i xylines[], Vec4i lineSeg[], int thresh,
		int size, int procMode, double& score, bool debug, int k, int l,
	...
	if (!isLine(pic, dd1, dx1, pt[0], pt[1], THRESHOLD, SIZE, 2, procMode,
			debug))
		return false;
	if (!isLine(pic, dd2, dx2, pt[2], pt[3], THRESHOLD, SIZE, 2, procMode,
			debug))
		return false;
	if (!isLine(pic, dd3, dx3, pt[0], pt[2], THRESHOLD, SIZE, 2, procMode,
			debug))
		return false;
	if (!isLine(pic, dd4, dx4, pt[1], pt[3], THRESHOLD, SIZE, 2, procMode,
			debug))
		return false;
...
void drawInnerBorder(Mat& src, double k, int x0, int y0) {
	...
	bool find = false;
	for (p1 = 0; p1 < 4; p1++) {
		if (points[p1].x >= 0 && points[p1].x <= src.cols && points[p1].y >= 0
				&& points[p1].y <= src.rows) {
			for (p2 = p1 + 1; p2 < 4; p2++) {
				if (points[p2].x >= 0 && points[p2].x <= src.cols
						&& points[p2].y >= 0 && points[p2].y <= src.rows) {
					find = true;
					break;
				}
			}
...
//	}
		...
		for (int i = 0; i < finalines.size(); i++) {
			...
			if (pt2.x != pt1.x)
				drawInnerBorder(dst,
						(0.0 + pt2.y - pt1.y) / (0.0 + pt2.x - pt1.x), pt1.x,
						pt1.y);
			else {
				pt1.y = 0;
				pt2.y = dst.rows;
...
int process(cv::Mat tsrc, Mat tslt, int procMode,
...
//				cout<<"here2"<<endl;
			...
			for (int sort_cur1 = 0; sort_cur1 < 4; sort_cur1++) {
				for (int sort_cur2 = sort_cur1 + 1; sort_cur2 < 4;
					...
					double angv1 = normalizeAngle(clines[ix1], width, height, k,
							l);
					//if(k==0&&l==13)
					//	std::cout<<"angle: "<<ix2<<std::endl;
					double angv2 = normalizeAngle(clines[ix2], width, height, k,
							l);
			...
			if (sort_arr[0] / 2 != sort_arr[1] / 2
				...
				if (isLikeRect(clines, debug)
						&& isRealQuadr(pic1, xylines, segs, THRESHOLD[1],
								SIZE[1], procMode, score, debug, k, l, qn)
						&& (procMode < 3
								&& ((angleSum < CV_PI / 6
